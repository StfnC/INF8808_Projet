...
def compute_medals_per_cat_per_country(df: pd.DataFrame, country_code: str, category: str) -> pd.DataFrame:
    count = 0
...
def preprocess():
    ...
    df_results = pd.DataFrame(columns=['country_code', 'best_category', *categories.keys()])
    for country in countries['features']:
        ...
        for category in categories:
            ...
            cat_results.append((compute_medals_per_cat_per_country(medals_df, name, category) / cat_max ) * 100)
    ...
    app.layout = html.Div([
        ...
        dcc.Dropdown(
            ...
            options=[{'label': cat, 'value': cat} for cat in categories.keys()] + [{'label': 'Global', 'value': 'global'}],
            value='global',
        ),
    ...
    def display_choropleth(category: str):
        if category == "global":
            fig = px.choropleth_map(df_results, geojson=countries, color="best_category", locations="country_code", featureidkey="properties.iso_a3")
            return fig
        else: 
            return px.choropleth_map(df_results, geojson=countries, color=category, locations="country_code", featureidkey="properties.iso_a3", color_continuous_scale="Reds")
    return app

if __name__ == "__main__":
    preprocess().run(debug=True, port=8050)